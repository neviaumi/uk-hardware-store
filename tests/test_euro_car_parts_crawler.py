...
async def test_car_parts_product_search(mock_server):
    mock_server.expect_request(
        "/eurocarparts/api/graphql", method="POST"
    ).respond_with_json(mock_response_json("vrm_lookup_euro_car_parts.json"))
    mock_server.expect_request("/eurocarparts/search/oil").respond_with_data(
        mock_response_data("product_search_euro_car_parts.html")
    )
    results = await euro_car_parts_crawler.car_parts_product_search("NX60OLA", "oil")
    assert isinstance(results, list)
    assert len(results) == 19
    first_item = results[0]
    assert first_item.title == "Sealey Oil Transfer Pump 12V"
...
@integration_test
async def test_euro_car_parts_live_car_parts_search():
    results = await euro_car_parts_crawler.car_parts_product_search("NX60OLA", "oil")
    assert isinstance(results, list)
    assert len(results) > 0
    first_item = results[0]
